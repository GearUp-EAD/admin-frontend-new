...
const Orders = () => {
  ...
  const [customerDetails, setCustomerDetails] = useState<{ [key: string]: Customer }>({});
  const [shippingDetail, setShippingDetail] = useState<{ [key: string]: Shipping }>({});
  ...
  const fetchShippingDetails = async (orderId: string) => {
    try {
      const response = await fetch(`http://localhost:8080/api/shippings/order/${orderId}`);
      return await response.json();
    } catch (error) {
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold">{orderCounts.processing}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold">{orderCounts.shipped}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm">
                ...
                <tr key={order.orderID}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <img
                      ...
                      src={customerDetails[order.customerId]?.imageUrl || '/default-avatar.png'}
                      alt={`${customerDetails[order.customerId]?.name || 'Customer'} profile`}
                    />
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">
                        {customerDetails[order.customerId]?.name || 'Loading...'}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {shippingDetail[order.orderID]?.shippingStatus || 'PENDING'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {shippingDetail[order.orderID]?.shippingAddress || 'No address'}
                  </td>
